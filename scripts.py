...
# ─── Your Credit Cards ─────────────────────────────────────
# Add each card you own. last_four must match what appears in bank SMS/email alerts.
cards:
  - bank: hdfc
    last_four: "1234"
    name: "HDFC Millennia"
...
# ─── Fetch Settings ────────────────────────────────────────
fetch:
  days_back: 90         # default lookback period
  max_emails_per_bank: 500
'''

# ─── .env.example ──────────────────────────────────────────
files['.env.example'] = '''# .env.example — copy this to .env and fill in your values
...
files['part1_email_reader.py'] = '''# part1_email_reader.py
...
# ─── dashboard.py ──────────────────────────────────────────
files['dashboard.py'] = '''# dashboard.py
...
# Email (stdlib — no install needed)
# imaplib, email, re, csv, json, os, hashlib, datetime
...
for fname in files:
    lines = files[fname].count('\n')
    print(f"   {fname:<35} ({lines} lines)")